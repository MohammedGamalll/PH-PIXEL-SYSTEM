...
export const Route = createFileRoute("/_authenticated/users/suppliers")({
...
function SuppliersPage() {
  const { t, dir } = useI18n();
  const { can } = useCan();
  const headStyle: React.CSSProperties = { backgroundColor: "#f9fafb", color: "#374151", padding: "8px 8px", fontWeight: 600, textAlign: dir === "rtl" ? "right" : "left", fontSize: 12, borderBottom: "1px solid #e5e7eb" };
  const cellStyle: React.CSSProperties = { borderBottom: "1px solid #f3f4f6", padding: "8px 8px", color: "#374151", fontSize: 13, textAlign: dir === "rtl" ? "right" : "left", overflow: "hidden", textOverflow: "ellipsis" };
  ...
  const initialCols: ColumnDef[] = useMemo(() => [
    ...
    { key: "opening_balance", label: t("users.table.opening_balance"), visible: false },
    { key: "advance_balance", label: t("users.table.advance_balance"), visible: false },
    { key: "created_at", label: t("users.table.created_at"), visible: false },
    { key: "address", label: t("users.table.address"), visible: false },
    { key: "unpaid_purchases", label: t("users.table.unpaid_purchases"), visible: true },
    { key: "purchase_returns", label: t("users.table.purchase_returns"), visible: false },
    { key: "total_due", label: t("users.table.total_due"), visible: true },
  ], [t]);
  ...
  const [chk, setChk] = useState({ debts: false, returns: false, advance: false, opening: false });

  const { data: rows = [] } = useContacts("supplier", { includeInactive: showInactive });
  const { data: balances } = useContactBalances();
  const del = useDeleteContact("supplier");
  ...
  const totalDueOf = (r: any) => dueOf(r).due;

  // One-shot backfill: re-allocate historical supplier payments so legacy
  // purchases get marked paid / partial. Idempotent — only allocates the
  // unallocated portion of each payment.
  const backfilledRef = useRef(false);
  useEffect(() => {
    if (backfilledRef.current) return;
    if (!balances || (rows as any[]).length === 0) return;
    const candidates = (rows as any[]).filter((r) => {
      const b: any = balances.get(r.id);
      return b && Number(b.payments_out || 0) > 0 && Number(b.unpaid_purchases || 0) > 0;
    });
    ...
    (async () => {
      try {
        ...
        for (const c of candidates) {
          try {
            await resettleContactDebt({ data: { contact_id: c.id, direction: "out" } });
          } catch { /* ignore per-contact failures */ }
        }
      } catch { /* ignore */ }
    })();
  }, [balances, rows]);
  ...
  const filtered = useMemo(
    () => (rows as any[]).filter((d) => {
      if (search && ![d.first_name, d.last_name, d.mobile, d.email, d.contact_id]
        .filter(Boolean).join(" ").includes(search)) return false;
      const anyChk = chk.debts || chk.returns || chk.advance || chk.opening;
      if (anyChk) {
        const b = balOf(d.id);
        const due = dueOf(d);
        const matches =
          (chk.debts && due.due > 0) ||
          (chk.returns && b.purchase_returns > 0) ||
  ...
  // Pre-sort: unpaid (due > 0) first, then the user's chosen sort
  const sorted = useMemo(() => {
    const arr = [...(tableSorted as any[])];
    arr.sort((a, b) => {
      const ad = totalDueOf(a) > 0 ? 0 : 1;
      const bd = totalDueOf(b) > 0 ? 0 : 1;
      return ad - bd;
    });
    return arr;
  ...
  const cellFor = (r: any, key: string) => {
    switch (key) {
      ...
      case "unpaid_purchases": return dueOf(r).due.toFixed(2);
      case "purchase_returns": return balOf(r.id).purchase_returns.toFixed(2);
      case "total_due": return totalDueOf(r).toFixed(2);
  ...
  const sumUnpaidPurchases = () => sorted.reduce((s, r: any) => s + dueOf(r).due, 0).toFixed(2);
  const sumPurchaseReturns = () => sorted.reduce((s, r: any) => s + balOf(r.id).purchase_returns, 0).toFixed(2);
  const sumAdvance = () => sorted.reduce((s, r: any) => s + dueOf(r).totalCredit, 0).toFixed(2);
  const sumTotalDue = () => sorted.reduce((s, r: any) => s + totalDueOf(r), 0).toFixed(2);

  return (
    <div className="space-y-3" dir={dir}>
      <PageHeader title={t("users.page.suppliers_title")} subtitle={t("users.page.suppliers_subtitle")} actions={
      ...
      <div className="flex flex-wrap items-center gap-3 px-2 py-2 rounded-md" style={{ backgroundColor: "#ffffff", border: "1px solid #e5e7eb" }}>
        <label className="inline-flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
          <input type="checkbox" checked={chk.debts} onChange={(e) => setChk((s) => ({ ...s, debts: e.target.checked }))} />
          ديون الموردين
        </label>
        <label className="inline-flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
      ...
      <DataCard>
        <TableToolbar
          ...
          onExportCsv={() => exportCsv("suppliers.csv")} onExportExcel={() => exportCsv("suppliers.xls")}
          printRef={printRef} printTitle="suppliers"
                  ...
                  <tr key={r.id} style={rowStyle}>
                    {visible.map((c) => c.key === "opt" ? (
                      <td key={c.key} style={cellStyle}>
                        <ContactOptionsMenu contact={r} scope="supplier" />
                      </td>
            ...
            {pageRows.length > 0 && (
              <tfoot>
                <tr style={{ backgroundColor: "#f9fafb", fontWeight: 600 }}>
                  {visible.map((c) => {
                    if (c.key === "opening_balance")
                      return <td key={c.key} style={cellStyle}>{sumKey(c.key)}</td>;
                    if (c.key === "advance_balance")
                      return <td key={c.key} style={cellStyle}>{sumAdvance()}</td>;
                    if (c.key === "unpaid_purchases")
                      return <td key={c.key} style={cellStyle}>{sumUnpaidPurchases()}</td>;
                    if (c.key === "purchase_returns")
                      return <td key={c.key} style={cellStyle}>{sumPurchaseReturns()}</td>;
                    if (c.key === "total_due")
                      return <td key={c.key} style={cellStyle}>{sumTotalDue()}</td>;
      ...
      <AddContactDialog open={open} onOpenChange={setOpen} defaultType="supplier" />
    </div>
  );
}

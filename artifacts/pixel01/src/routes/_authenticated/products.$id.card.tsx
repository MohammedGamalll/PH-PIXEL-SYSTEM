...
function ItemCardPage() {
  ...
  const fmtDate = (s?: string | null) =>
    s ? new Date(s).toLocaleString(locale, { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" }) : "";
  ...
  const { data: bundle } = useQuery({
    ...
    enabled: !!user,
    queryFn: async () => {
      const [pItems, iItems, dItems, purchases, invoices, damaged, contacts, suppliers, pReturns, btSrc, btTgt, srItems, srHeaders] = await Promise.all([
        (supabase.from("purchase_items") as any).select("id,purchase_id,quantity,base_quantity,unit_name,unit_price,total,expiry_date").eq("product_id", id),
        (supabase.from("invoice_items") as any).select("id,invoice_id,quantity,base_quantity,unit_name,unit_price,total,expiry_date").eq("product_id", id),
        (supabase.from("damaged_stock_items") as any).select("id,damaged_stock_id,quantity,base_quantity,unit_name,unit_price,total,expiry_date").eq("product_id", id),
        (supabase.from("purchases") as any).select("id,purchase_number,ref_no,purchase_date,supplier_id,supplier_name_snapshot,notes,created_at,is_opening,created_by_name_snapshot"),
        (supabase.from("invoices") as any).select("id,invoice_number,issue_date,type,is_returned_from_id,customer_id,customer_name_snapshot,created_at,created_by_name_snapshot"),

        (supabase.from("damaged_stock") as any).select("id,ref_number,damage_date,damage_type,reason,created_at"),
        (supabase.from("contacts") as any).select("id,first_name,last_name,business_name"),
        (supabase.from("suppliers") as any).select("id,name"),
        (supabase.from("purchase_returns") as any).select("id,ref_no,return_date,purchase_id,total_amount,created_at"),
        (supabase.from("inventory_branch_transfer_items") as any)
          .select("id,transfer_id,quantity,base_quantity,unit_name,inventory_branch_transfers!inner(id,transfer_date,created_at,target_name_snapshot,created_by_name_snapshot)")
          .eq("source_product_id", id),
        (supabase.from("inventory_branch_transfer_items") as any)
          .select("id,transfer_id,quantity,base_quantity,unit_name,inventory_branch_transfers!inner(id,transfer_date,created_at,target_name_snapshot,created_by_name_snapshot)")
          .eq("target_product_id", id),
        (supabase.from("standalone_return_items") as any)
          .select("id,standalone_return_id,quantity,unit_price,total,expiry_date")
          .eq("product_id", id),
        (supabase.from("standalone_returns") as any)
          .select("id,reference_no,return_type,return_date,reason,created_at,created_by_name_snapshot"),
      ]);
      for (const r of [pItems, iItems, dItems, purchases, invoices, damaged, pReturns]) {
  ...
  const expiryCell = (iso: string | null | undefined) => {
    ...
    if (diffDays <= EXPIRY_WARN_DAYS) return `⚠️ ${label} (${t("products.card.expiry_warn")})`;
    return label;
  };

  const { rows, stats } = useMemo(() => {
    const emptyStats = { purchased: 0, sold: 0, salesReturn: 0, purchaseReturnCount: 0, damaged: 0, current: 0 };
    if (!bundle || !product) return { rows: [] as Movement[], stats: emptyStats };

    const purchMap = new Map<string, any>(bundle.purchases.map((p: any) => [p.id, p]));
    const invMap = new Map<string, any>(bundle.invoices.map((i: any) => [i.id, i]));
    const dmgMap = new Map<string, any>(bundle.damaged.map((d: any) => [d.id, d]));
    const partyMap = new Map<string, string>();
    for (const c of bundle.contacts as any[]) {
      partyMap.set(c.id, c.business_name || [c.first_name, c.last_name].filter(Boolean).join(" "));
    }
    ...
    for (const it of bundle.pItems as any[]) productPurchaseIds.add(it.purchase_id);

    const out: Movement[] = [];
    let purchased = 0, sold = 0, salesReturn = 0, purchaseReturnCount = 0, damaged = 0;
    ...
    for (const it of bundle.pItems as any[]) {
      ...
      out.push({
        ...
        party: p.supplier_name_snapshot || (p.supplier_id ? (supMap.get(p.supplier_id) || partyMap.get(p.supplier_id) || "") : "") || "بدون مورد",
        qty: baseQty,
    ...
    for (const it of bundle.iItems as any[]) {
      ...
      if (isReturn) salesReturn += baseQty; else sold += baseQty;
      out.push({
        id: `i-${it.id}`,
        type: isReturn ? t("products.card.sale_return") : t("products.card.sale"),
        date: fmtDate(inv.created_at ?? inv.issue_date),
        ref: inv.invoice_number || "",
        invoiceId: inv.id,
        party: inv.customer_name_snapshot || (inv.customer_id ? (partyMap.get(inv.customer_id) || "") : "") || "نقدي",
        qty: baseQty,
        qtyLabel: `${Math.abs(Number(it.quantity ?? 0))} ${it.unit_name || ""}`.trim(),
    ...
    for (const it of bundle.dItems as any[]) {
      ...
      const baseQty = Number(it.base_quantity ?? it.quantity ?? 0);
      damaged += baseQty;
      const isAdjustment = d?.damage_type === "stock_adjustment" || String(d?.ref_number || "").startsWith("ADJ-");
      out.push({
        id: `d-${it.id}`,
    ...
    const srHdrMap = new Map<string, any>(((bundle as any).srHeaders || []).map((h: any) => [h.id, h]));
    let standaloneSalesRet = 0, standalonePurchRet = 0;
    for (const it of ((bundle as any).srItems || []) as any[]) {
      ...
      if (isSales) standaloneSalesRet += qty; else standalonePurchRet += qty;
      out.push({
        id: `sr-${it.id}`,
        type: isSales ? `${t("products.card.sale_return")} (حر)` : `${t("products.card.purchase_return")} (حر)`,
    ...
    for (const it of (bundle as any).btSrc as any[]) {
      ...
      out.push({
        ...
        ref: "",
        party: h.target_name_snapshot || (lang === "ar" ? "فرع آخر" : "Other branch"),
        qty: baseQty,
        qtyLabel: `${Number(it.quantity ?? 0)} ${it.unit_name || ""}`.trim(),
        delta: -baseQty,
        expiry: "—",
    ...
    for (const it of (bundle as any).btTgt as any[]) {
      ...
      out.push({
        ...
        date: fmtDate(h.created_at ?? h.transfer_date),
        ref: "",
        party: h.created_by_name_snapshot || (lang === "ar" ? "فرع آخر" : "Other branch"),
        qty: baseQty,
        qtyLabel: `${Number(it.quantity ?? 0)} ${it.unit_name || ""}`.trim(),
        delta: +baseQty,
        expiry: "—",
    ...
    const typeRank: Record<string, number> = { [t("products.card.opening")]: 0, [t("products.card.purchase")]: 1, [t("products.card.sale_return")]: 2, [t("products.card.purchase_return")]: 3, [t("products.card.sale")]: 4, [t("products.card.damaged")]: 5 };
    out.sort((a, b) => {
    ...
    void transferIn; void transferOut;
  ...
  const fmtBase = (n: number) => (product ? formatBaseQuantity(n, product) : `${n}`);
  ...
  const subUnits = product
    ? [
        product.sub_unit_1 ? `${product.sub_unit_1}${product.sub_unit_1_ratio ? ` (×${product.sub_unit_1_ratio})` : ""}` : null,
        product.sub_unit_2 ? `${product.sub_unit_2}${product.sub_unit_2_ratio ? ` (×${product.sub_unit_2_ratio})` : ""}` : null,
      ].filter(Boolean).join(" / ")
    : "";
  ...
  const { data: openInvoice } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase.from("invoices") as any)
        .select("*").eq("id", openInvoiceId).maybeSingle();
      if (error) throw error;
      return data;
    },
  });
  const { data: openInvoiceCustomer } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await (supabase.from("contacts") as any)
        ...
        .eq("id", openInvoice.customer_id).maybeSingle();
      return data;
    },
  });
  const openCustomerName = openInvoice
    ? (openInvoice.customer_name_snapshot
        || [openInvoiceCustomer?.first_name, openInvoiceCustomer?.last_name].filter(Boolean).join(" ").trim()
  ...
  const { data: openPurchase } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase.from("purchases") as any)
        .select("*").eq("id", openPurchaseId).maybeSingle();
      if (error) throw error;
      return data;
    },
  });
  const { data: openPurchaseSupplier } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await (supabase.from("suppliers") as any)
        .select("name").eq("id", openPurchase.supplier_id).maybeSingle();
      return data;
    },
  });
  const openSupplierName = openPurchase
    ? (openPurchase.supplier_name_snapshot || openPurchaseSupplier?.name || "بدون مورد")
    : "";
  ...
  return (
    <div className="space-y-3" dir={dir}>
      <PageHeader
        ...
        subtitle={product ? `${product.name}${product.sku ? ` — ${product.sku}` : ""}` : ""}
        actions={
          <Link to="/products" className="h-9 px-4 rounded-md text-sm flex items-center gap-2"
      ...
      {product && (
        <DataCard>
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-shrink-0 flex items-center justify-center w-28 h-28 rounded-md overflow-hidden"
            ...
            <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-2 text-sm">
              <Info label={t("products.card.category")} value={(product as any).categoryName || "—"} />
              <Info label={t("products.card.brand")} value={(product as any).brandName || "—"} />
              <Info label="SKU" value={product.sku || "—"} />
              <Info label={t("products.card.low_threshold")} value={String(product.low_stock_threshold ?? 0)} />
              <Info label={t("products.card.unit_main")} value={product.main_unit || product.unit || "—"} />
              <Info label={t("products.card.units_sub")} value={subUnits || "—"} />
              <Info label={t("products.card.price_now")} value={String(product.price ?? 0)} />
              <Info label={t("products.card.cost_now")} value={String(product.cost ?? 0)} />
              <Info label={t("products.card.has_expiry")} value={product.has_expiry ? t("products.card.yes") : t("products.card.no")} />
              <Info
                label="الضمان"
                value={
                  product.warranty
                    ? `${product.warranty.name} — ${product.warranty.duration} ${
                        { day: "يوم", month: "شهر", year: "سنة" }[product.warranty.duration_unit as string] ||
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
        <StatCard label={t("products.card.total_purchased")} value={fmtBase(stats.purchased)} accent="#10b981" />
        <StatCard label={t("products.card.total_sold")} value={fmtBase(stats.sold)} accent="#3b82f6" />
        <StatCard label={t("products.card.returned_in")} value={fmtBase(stats.salesReturn)} accent="#f59e0b" />
        <StatCard label={t("products.card.returned_out")} value={String(stats.purchaseReturnCount)} accent="#f59e0b" />
        <StatCard label={t("products.card.damaged_total")} value={fmtBase(stats.damaged)} accent="#ef4444" />
      </div>
      {product && (() => {
        ...
        const label = isOut ? "⛔ نفذ المخزون" : isLow ? `⚠️ المخزون منخفض (حد التنبيه: ${threshold})` : `المخزون متوفر (حد التنبيه: ${threshold})`;
        return (
          <div className="grid grid-cols-1 gap-3">
            <div className="rounded-lg p-4 flex items-center justify-between gap-3 flex-wrap"
              style={{ backgroundColor: "#ffffff", borderRight: `4px solid ${accent}`, boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
              <div className="flex flex-col gap-1">
                <div className="text-xs" style={{ color: "#6b7280" }}>{t("products.card.current_stock")}</div>
                <div className="text-xl font-bold" style={{ color: "#111827" }}>{fmtBase(stats.current)}</div>
              </div>
              <span className="px-3 py-1.5 rounded-md text-xs font-semibold"
                style={{ backgroundColor: bg, color: fg }}>{label}</span>
            </div>
          </div>
        );
      })()}
            ...
            if (k === "ref" && r.ref && r.invoiceId) {
              return (
                <button
                  ...
                  style={{ color: "#2563eb", textDecoration: "underline", fontWeight: 600, background: "transparent", border: "none", padding: 0, cursor: "pointer" }}
                >
                  {r.ref}
                </button>
              );
...
function Info({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <div className="font-medium" style={{ color: "#111827" }}>{value}</div>
    </div>
  );
}

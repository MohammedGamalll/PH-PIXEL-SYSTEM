...
import { useCategories, useCreateCategory, useUpdateCategory, useDeleteCategory } from "@/hooks/use-product-meta";
...
export const Route = createFileRoute("/_authenticated/products/categories")({ component: CategoriesPage });
...
function CategoriesPage() {
  ...
  const [cols, setCols] = useState<ColumnDef[]>(() => [
    { key: "name", label: t("products.categories.col.name"), visible: true },
    { key: "code", label: t("products.categories.col.code"), visible: true },
    { key: "description", label: t("products.categories.col.desc"), visible: true },
  ...
  const fields: FieldDef[] = [
    { type: "text", key: "name", label: t("products.categories.name"), required: true, placeholder: t("products.categories.name") },
    { type: "text", key: "code", label: t("products.categories.code"), placeholder: t("products.categories.code") },
    { type: "textarea", key: "description", label: t("products.categories.desc"), placeholder: t("products.categories.desc") },
  ...
  const onSubmit = async (values: Record<string, any>) => {
    const payload = {
      ...
      has_sub_category: !!values.has_sub_category,
    };
    ...
    <div className="space-y-3" dir={dir}>
      ...
      <DataCard>
        <TableToolbar search={search} onSearchChange={setSearch} perPage={perPage} onPerPageChange={setPerPage}
          onExportCsv={() => exportCsv("categories.csv")} onExportExcel={() => exportCsv("categories.xls")}
      ...
      <EntityDialog open={open} onOpenChange={(v) => { setOpen(v); if (!v) setEditing(null); }}
        title={editing ? t("products.categories.edit_title") : t("products.categories.add_title")} fields={fields}
        initial={editing ?? { has_sub_category: false }} onSubmit={onSubmit}

...
import { AddCustomerGroupDialog } from "@/components/contacts/AddCustomerGroupDialog";
import { ContactFilters } from "@/components/contacts/ContactFilters";
import { useI18n } from "@/lib/i18n";
import { useTableSort } from "@/components/shared/useTableSort";
import { SortableHead } from "@/components/shared/SortableHead";

export const Route = createFileRoute("/_authenticated/users/customer-groups")({
...
function CustomerGroupsPage() {
  ...
  const initialCols: ColumnDef[] = useMemo(() => [
    ...
    { key: "name", label: t("users.cg.name").replace(":", ""), visible: true },
    { key: "amount", label: t("users.table.percentage"), visible: true },
    { key: "price_group", label: t("users.table.price_group"), visible: true },
  ], [t]);
  ...
  const { data: rows = [] } = useCustomerGroups();
  const { data: pgs = [] } = usePriceGroups();
  const del = useDeleteCustomerGroup();
  const pgName = (id?: string) => (pgs as any[]).find((p) => p.id === id)?.name ?? "";

  useEffect(() => setCols(initialCols), [initialCols]);

  const filtered = useMemo(() => (rows as any[]).filter((d) => !search || (d.name ?? "").includes(search)), [rows, search]);
  ...
  const cellFor = (r: any, key: string) => {
    if (key === "amount") return Number(r.amount ?? 0).toFixed(2);
    if (key === "price_group") return pgName(r.price_group_id);
    return r[key] ?? "";
  };
  ...
  return (
    <div className="space-y-3" dir={dir}>
      <PageHeader title={t("users.page.customer_groups_title")} actions={
      ...
      <DataCard>
        <TableToolbar
          ...
          onExportCsv={() => exportCsv("customer-groups.csv")} onExportExcel={() => exportCsv("customer-groups.xls")}
          printRef={printRef} printTitle="customer-groups"
      ...
      <AddCustomerGroupDialog open={open} onOpenChange={setOpen} />
    </div>
  );
}

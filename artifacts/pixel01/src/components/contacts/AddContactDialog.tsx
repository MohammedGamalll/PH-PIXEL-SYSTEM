...
import { z } from "zod";
import { useCreateContact, type ContactType } from "@/hooks/use-contacts";
...
export function AddContactDialog({
  ...
  const [cf, setCf] = useState<string[]>(Array(10).fill(""));
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (!parsed.success) {
      ...
      const fieldLabels: Record<string, string> = {
        first_name: t("users.contact.first_name"),
        mobile: t("users.contact.mobile"),
        email: t("users.contact.email"),
        opening_balance: t("users.contact.opening"),
        credit_limit: t("users.contact.credit_limit"),
      };
      toast.error(`${fieldLabels[String(field)] ?? field}: ${issue?.message ?? ""}`.trim() || t("users.contact.toast_required"));
      return;
    }
    const payTermStr = payTerm ? `${payTerm} ${payTermUnit}` : null;
    await create.mutateAsync({
      ...
      custom_field_1: cf[0].trim() || null,
      custom_field_2: cf[1].trim() || null,
      custom_field_3: cf[2].trim() || null,
      custom_field_4: cf[3].trim() || null,
      custom_field_5: cf[4].trim() || null,
      custom_field_6: cf[5].trim() || null,
      custom_field_7: cf[6].trim() || null,
      custom_field_8: cf[7].trim() || null,
      custom_field_9: cf[8].trim() || null,
      custom_field_10: cf[9].trim() || null,
    });
    reset();
    onOpenChange(false);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-start" style={{ color: DARK }}>
        ...
        <form onSubmit={submit} className="space-y-3 form-strong">
            ...
            <div className="flex items-end gap-3">
              <label className="flex items-center gap-1 text-sm" style={{ color: "#374151" }}>
                <input
                  type="radio"
                  name="bt"
                  checked={businessType === "person"}
                  onChange={() => setBusinessType("person")}
                />{" "}{t("users.contact.person")}
              </label>
              <label className="flex items-center gap-1 text-sm" style={{ color: "#374151" }}>
                <input
                  type="radio"
          ...
          {moreOpen && (
            <div className="space-y-3 p-3" style={{ border: "1px solid #e5e7eb", borderRadius: 6 }}>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                <div>
                  <label style={labelStyle}>{t("users.contact.tax_number")}</label>
                  <input style={inputStyle} value={taxNumber} onChange={(e) => setTaxNumber(e.target.value)} />
                </div>
                <div>
                  <label style={labelStyle}>{t("users.contact.opening")}</label>
                  <input type="number" style={inputStyle} value={opening} onChange={(e) => setOpening(e.target.value)} />
                </div>
                <div>
                ...
                <div>
                  <label style={labelStyle}>{t("users.contact.assigned_to")}</label>
                  <input style={inputStyle} value={assignedTo} onChange={(e) => setAssignedTo(e.target.value)} />
                </div>
                <div>
                  <label style={labelStyle}>{t("users.contact.credit_limit")}</label>
                  <input type="number" style={inputStyle} value={creditLimit} onChange={(e) => setCreditLimit(e.target.value)} />
                </div>
                <div>
                  <label style={labelStyle}>{t("users.contact.phone")}</label>
                  <input style={inputStyle} value={phone} onChange={(e) => setPhone(e.target.value)} />
                </div>
                <div>
                  <label style={labelStyle}>{t("users.contact.alt_mobile")}</label>
                  <input style={inputStyle} value={altMobile} onChange={(e) => setAltMobile(e.target.value)} />
                </div>

                {businessType === "person" && (
                  <>
                    <div>
                      <label style={labelStyle}>{t("users.contact.prefix")}</label>
                      <input style={inputStyle} value={prefix} onChange={(e) => setPrefix(e.target.value)} />
                    </div>
                    <div>
                      <label style={labelStyle}>{t("users.contact.middle_name")}</label>
                      <input style={inputStyle} value={middleName} onChange={(e) => setMiddleName(e.target.value)} />
                    </div>
                    <div>
                      <label style={labelStyle}>{t("users.contact.dob")}</label>
                      <DateInput value={dob} onChange={setDob} style={inputStyle} />
                    </div>
                  </>
                )}
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-2 gap-3">
                <div>
                  <label style={labelStyle}>{t("users.contact.addr1")}</label>
                  <input style={inputStyle} value={addr1} onChange={(e) => setAddr1(e.target.value)} />
                </div>
                <div>
                  <label style={labelStyle}>{t("users.contact.addr2")}</label>
                  <input style={inputStyle} value={addr2} onChange={(e) => setAddr2(e.target.value)} />
                </div>
              </div>

              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div>
                  <label style={labelStyle}>{t("users.contact.district")}</label>
                  <input style={inputStyle} value={district} onChange={(e) => setDistrict(e.target.value)} />
                </div>
                <div>
                  <label style={labelStyle}>{t("users.contact.city")}</label>
                  <input style={inputStyle} value={city} onChange={(e) => setCity(e.target.value)} />
                </div>
                <div>
                  <label style={labelStyle}>{t("users.contact.state")}</label>
                  <input style={inputStyle} value={state} onChange={(e) => setState(e.target.value)} />
                </div>
                <div>
                  <label style={labelStyle}>{t("users.contact.zip")}</label>
              ...
              {type !== "supplier" && (
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                  {cf.map((v, i) => (
                    <div key={i}>
                      <label style={labelStyle}>{t("users.contact.cf_label", { n: i + 1 })}</label>
                      <input
                        style={inputStyle}
                        value={v}
                        onChange={(e) => setCfAt(i, e.target.value)}
                      />
                    </div>
                  ))}
                </div>
          ...
          <DialogFooter className="flex-row-reverse sm:flex-row-reverse gap-2">
            <button
              ...
              className="h-10 px-5 rounded-md text-white text-sm"

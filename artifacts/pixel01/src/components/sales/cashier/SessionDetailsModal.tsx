...
export function SessionDetailsModal({ sessionId, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: invs } = await (supabase.from("invoices") as any)
        .select("id, total, payment_method, type, status")
        .eq("session_id", sessionId)
        .in("type", ["sale", "sale_return"]);
      const all = ((invs ?? []) as any[]).filter((x) => x.status !== "void" && x.status !== "cancelled");
      ...
      setStat({
        openingCash: Number(sess?.opening_cash || 0),
        openedAt: sess?.opened_at || new Date().toISOString(),
  ...
  const totalRefunds = (stat?.cashRefunds ?? 0) + (stat?.cardRefunds ?? 0) + (stat?.bankRefunds ?? 0) + (stat?.creditRefunds ?? 0) + (stat?.multiRefunds ?? 0);
  ...
  const dateRange = stat
    ? `( ${formatDateTime(new Date(stat.openedAt))} - ${formatDateTime(new Date())} )`
    : "";
  ...
  return (
    <Win7Modal title={`تفاصيل الجلسة ${dateRange}`} onClose={onClose} width={780}>
      {!stat ? (
        ...
        <div dir="rtl" style={{ display: "grid", gap: 12, fontSize: 13, padding: 4 }}>
          <table style={tbl}>
            <tbody>
              <tr style={{ background: "#dbeafe", fontWeight: 700 }}>
                <td style={tdStyle}>عدد الفواتير</td>
                <td style={{ ...tdStyle, textAlign: "left" }}>{stat.invoiceCount}</td>
              </tr>
              <tr style={{ background: "#d1fae5", fontWeight: 700 }}>
                <td style={tdStyle}>إجمالي المبيعات</td>
                <td style={{ ...tdStyle, textAlign: "left" }}>{fmt(totalShift)} ج.م</td>
              </tr>
              <tr style={{ background: "#fde2e2", fontWeight: 700 }}>
                <td style={tdStyle}>إجمالي المرتجعات ({stat.returnCount})</td>
                <td style={{ ...tdStyle, textAlign: "left", color: "#b91c1c" }}>-{fmt(totalRefunds)} ج.م</td>
              </tr>
              <tr style={{ background: "#fef3c7", fontWeight: 800 }}>
                <td style={tdStyle}>الصافي</td>
                <td style={{ ...tdStyle, textAlign: "left" }}>{fmt(netSales)} ج.م</td>
              </tr>
            </tbody>
          </table>
          <table style={tbl}>
            ...
            <tbody>
              <Row label="النقدية في الدرج:" sales={fmt(stat.openingCash)} exp="--" />
              ...
              <tbody>
                {stat.items.length === 0 ? (
                  ...
                  <tr key={i}>
                    ...
                    <td style={tdStyle}>{fmt(it.total)} ج.م</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr style={{ background: "#d1fae5", fontWeight: 700 }}>
                  <td style={tdStyle}>#</td>
                  <td colSpan={2} style={tdStyle}>المبلغ الإجمالي: {fmt(itemsTotal)} ج.م</td>
                  <td style={tdStyle}>{itemsQty.toFixed(0)}</td>
                  <td style={tdStyle}>{fmt(itemsTotal)} ج.م</td>
                </tr>
                ...
                <tbody>
                  {stat.returnedItems.map((it, i) => (
                    <tr key={i} style={{ background: "#fef2f2" }}>
                      ...
                      <td style={{ ...tdStyle, color: "#b91c1c" }}>{fmt(it.total)} ج.م</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr style={{ background: "#fde2e2", fontWeight: 700 }}>
                    <td style={tdStyle}>#</td>
                    <td colSpan={2} style={tdStyle}>إجمالي المرتجعات ({stat.returnCount} فاتورة): {fmt(returnedTotal)} ج.م</td>
                    <td style={tdStyle}>{returnedQty.toFixed(0)}</td>
                    <td style={{ ...tdStyle, color: "#b91c1c" }}>-{fmt(returnedTotal)} ج.م</td>
              ...
              <tbody>
                {brandAgg.length === 0 ? (
                  ...
                  <tr key={i}>
                    ...
                    <td style={tdStyle}>{fmt(b.total)} ج.م</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr style={{ background: "#d1fae5", fontWeight: 700 }}>
                  <td style={tdStyle}>#</td>
                  <td style={tdStyle}>المبلغ الإجمالي: {fmt(itemsTotal)} ج.م</td>
                  <td style={tdStyle}>{itemsQty.toFixed(0)}</td>
                  <td style={tdStyle}>{fmt(itemsTotal)} ج.م</td>
          ...
          <div style={{ display: "flex", gap: 6, justifyContent: "flex-start", paddingTop: 4, borderTop: "1px solid #e5e7eb" }}>
            <button
              onClick={onClose}
              style={{ padding: "6px 18px", background: "#1f2937", color: "#fff", border: "1px solid #111827", borderRadius: 3, cursor: "pointer", fontWeight: 600 }}
            >
              إلغاء
            </button>
            <button
              onClick={handlePrint}
              style={{ padding: "6px 18px", background: "linear-gradient(180deg, #6366f1, #4338ca)", color: "#fff", border: "1px solid #3730a3", borderRadius: 3, cursor: "pointer", fontWeight: 700 }}
            >
              طباعة
            </button>
          </div>
...
const tbl: React.CSSProperties = { width: "100%", borderCollapse: "collapse", border: "1px solid #cbd5e1" };
const thStyle: React.CSSProperties = { padding: "6px 8px", background: "#f3f4f6", borderBottom: "1px solid #cbd5e1", textAlign: "right", fontWeight: 700, fontSize: 12 };
const tdStyle: React.CSSProperties = { padding: "5px 8px", borderBottom: "1px solid #e5e7eb", textAlign: "right", fontSize: 12 };
